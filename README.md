...
- [🚀 Установка окружения](#section-installation)
...
## ⚙️ Структура проекта
```
└── itmo_project /                  # Корневая папка проекта
    ├── README.md                   # Детальное описание работы с проектом 
    ├── environment.yml             # Список библиотек и зависимостей для установки
    ├── requirements.txt            # Список библиотек и зависимостей для установки
    ...
    │   ├── environment.yml         # Список библиотек и зависимостей для установки
    │   ├── README.md               # Детальное описание работы с API
...
Если у вас установлена Conda:
```bash
conda env create -f environment.yml # cоздание окружения из environment.yml
conda activate dde_itmo # aктивация окружения
```
### Способ 2: Использование pip
Если у вас нет Conda или предпочитаете pip:
```bash
...
Анализ проводится в ноутбуке `notebooks/EDA.ipynb`
...
<a id="section-etl"></a>
## 🔄 ETL-пакет
ETL-пакет состоит из четырёх модулей: extract, transform, load и validate
### 1. extract
**Файл:** `etl/extract.py`
...
* Коррекция давлений - исправление записей с некорректным соотношением систолического и диастолического давлений, с пометкой случаев, требующих дополнительной проверки
...
Этот модуль реализует сохранение трансформированных данных в формате .parquet в папку `data/processed/disease_risk_processed.parquet`, а так же загрузку первых 100 строк в таблицу «your_table_name» (в моем случае таблица называется «kalashnikova») в базу данных PostgreSQL
...
Этот модуль релизует проверки операций с датасетом (в случае, если какая-то операция пройдет неуспешно, информация об этом поступит из этого модуля). Модуль проверяет:
...
--max-rows 100 - агрумент функции, показывающий максимальное количество строк для загрузки в БД

Вывод команды в терминале: 
```bash
Запуск ETL процесса...
Максимальное количество строк для БД: 100
--------------------------------------------------
1. Загрузка данных...
Файл загружен. Файл содержит 4000 строк.
Сырые данные сохранены: data/raw/disease_risk_dataset.csv
✅ Данные успешно загружены

2. Преобразование данных...
Найдено 26 строк с BMI < 15
Удалено 26 строк с нереалистичным BMI
Осталось строк: 3974 из 4000
Найдено 338 записей с некорректным соотношением давлений
  - Исправлено заменой: 321 записей
  - Осталось равных давлений: 17 записей
'Alcohol_Consumption': заполнено 1628 пропусков
'Previous_Diagnosis': заполнено 2013 пропусков
✅ Преобразование данных завершено

3. Сохранение и загрузка данных в БД...
Данные сохранены в parquet: data/processed/disease_risk_processed.parquet
Подключение к базе данных установлено
Подготовлено 100 строк для загрузки в БД
Данные успешно записаны в базу
Загрузка данных успешно завершена!
✅ Данные сохранены и загружены в БД

4. Проверка данных...
Запуск полной валидации ETL процесса
------------------------------------------------------------
Файл в data/raw существует. Размер файла: 292941
В Dataset НЕТ записей с BMI < 15 :)
Пропусков НЕТ :)
Соотношение давлений корректно :)
Age: int32 ✅
BMI: float32 ✅
...
Parquet файл существует!
Файл непустой
Размер файла: 70946 байт
Данные в parquet: 3974 строк, 16 колонок
В базе записано 100 строк (все верно!)
------------------------------------------------------------
