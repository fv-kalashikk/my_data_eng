...
- [🔗 Информация и ссылки на датасет](#информация-и-ссылки-на-датасет)
- [⚙️ Структура проекта](#структура-проекта)
- [🚀 Установка](#установка)
- [📊 Разведочный анализ данных (EDA)](#разведочный-анализ-данных-(EDA))
- [🔄 ETL-пакет](#ETL-пакет)
- [🖇 Ответвление проекта](#ответвление-проекта)
...
- **Распределение целевой переменной**: баланс классов в колонке `Disease_Risk`
...
* Коррекция давления - исправление некорректных соотношений систолического/диастолического
* Обработка пропусков - заполнение "Unknown" для категориальных переменных
...
По выполнению этого модуля датасет принял следущий вид:
...
### 3. 📤 load.py
**Файл:** `etl/load.py`
Этот модуль реализует сохранение трансформированных данных в формате .parquet в папку data/processed/disease_risk_processed.parquet, а так же загрузку первых 100 строк в таблицу kalashnikova в базу данных PostgreSQL
